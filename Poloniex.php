<?php
	...
	class Poloniex {
		...
		public function __construct() {
			$json = file_get_contents('secret.json');
			$apiObject = json_decode($json);
			$this->api_key = $apiObject->api_key;
			$this->api_secret = $apiObject->api_secret;
		}
    ...
    public function create_loan_offer($loanAmount, $rate, $currency,$duration = 2, $autoRenew = 0) {
      $rate = $rate/100;
      return $this->query(
        array(
    ...
    public function cancel_loan_offer($id) {
      return $this->query(
        array(
    ...
    public function get_loan_orders($currency) {
			$trades = $this->retrieveJSON($this->public_url.
        '?command=returnLoanOrders&currency='.strtoupper($currency));
			return $trades;
		}
